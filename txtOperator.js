...
const regexFun = (value, name) => {
  if (value.includes("Regex")) {
    return `!(${value}.test(formData.${name
      .split(" ")
      .join("")
      .toLowerCase()}))`;
  } else {
    map = { eq: "===", gt: ">", lt: "<", lteq: "<==" };
    v = value.split(/[.()]/);
    return `!(formData.${name.split(" ").join("").toLowerCase()} ${
      map[v[1].trim()]
    } formData.${v[2]})`;
  }
};
...
const takeValue = (datav) => {
  return [
    datav.slice(0, datav.indexOf("(")),
    datav.slice(datav.indexOf("(") + 1, datav.lastIndexOf(")")),
  ];
};
...
const getLogic = (data) => {
  ...
  for (let i = 0; i < data.length; i++) {
    if (data[i].includes("redirect")) {
      ...
      data[i].includes("showsection") ||
      data[i].includes("hidesection")
    ) {
      state2[
        takeValue(data[i])[1][0].toUpperCase() +
          takeValue(data[i])[1].slice(1) +
          "State"
      ] = v[takeValue(data[i])[0]];
